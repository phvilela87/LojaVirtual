...
import dm114.br.inatel.pvilela.lojavirtual.models.AccessToken;
import dm114.br.inatel.pvilela.lojavirtual.util.WSConstants;
import dm114.br.inatel.pvilela.lojavirtual.util.WSUtil;

/**
 * Created by pedro on 20/06/16.
 */
public class WebServiceUserMessageProviderClient {
    private static final String GET_TOKEN = "/api/token";
    private static final String TAG = "WebServiceUserMessageProviderClient";
    private static HttpURLConnection conn;
    private static AccessToken accessToken;

    private WebServiceUserMessageProviderClient() {
    }
    ...
    public static boolean isTokenValid(Context context) {
        ...
        if (sharedSettings.contains(
                context.getString(R.string.pref_ws_user_message_provider_access_token)) &&
                sharedSettings.contains(context.getString(
                        R.string.pref_ws_user_message_provider_access_token_expiration))) {
            expirationTime = sharedSettings.getLong(context.getString(
                    R.string.pref_ws_user_message_provider_access_token_expiration), 0);
    ...
    private static void invalidateToken(Context context) {
        ...
        editor.remove(context.getString(R.string.pref_ws_user_message_provider_access_token));
        editor.remove(context.getString(R.string.pref_ws_user_message_provider_access_token_expiration));
        editor.commit();
    }
    ...
    private static WebServiceResponse authenticate(Context context) {
        ...
        String baseAddress = WSUtil.getMessageProviderHostAddress(context);
        accessToken = null;
        try {
            ...
            String wsUsername = sharedSettings.getString(
                    ...
                    context.getString(R.string.pref_ws_user_username));
            String wsPassword = sharedSettings.getString(
                    context.getString(R.string.pref_user_password),
                    context.getString(R.string.pref_ws_user_password));
            ...
            if (conn.getResponseCode() == 200) {
                ...
                editor.putString(context.getString(
                        R.string.pref_ws_user_message_provider_access_token),
    ...
    public static WebServiceResponse loginAuthentication(Context context, String email, String password) {
        WebServiceResponse webServiceResponse = new WebServiceResponse();
        String baseAddress = WSUtil.getMessageProviderHostAddress(context);
        accessToken = null;
        try {
    ...
    public static WebServiceResponse delete(Context context,
                                         String host) {
        WebServiceResponse webServiceResponse = new WebServiceResponse();
        try {
            webServiceResponse = init(context, host,
